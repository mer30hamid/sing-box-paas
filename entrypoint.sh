...
if [[ ! -n "$CORE_URL" ]]; then
  ...
  wget -O $nx.tar.gz https://github.com/SagerNet/sing-box/releases/download/v${VER}/sing-box-${VER}-linux-amd64.tar.gz --no-check-certificate
else
  wget -O $nx.tar.gz $CORE_URL --no-check-certificate
fi
